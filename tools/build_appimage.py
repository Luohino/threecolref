...
# Build the threecolref appimage. Run from the git root directory.
# On github actions:
...
parser = argparse.ArgumentParser(
    description=('Create an appimage for threecolref. '
                 'Run from the git root directory.'))
parser.add_argument(
    '-v', '--version',
    required=True,
    help='threecolref version number/tag for output file')
parser.add_argument(
    '-j', '--jsonfile',
    required=True,
...
runbee = f'"$APPDIR/opt/python{PYVER}/bin/python{PYVER}" -I -m threecolref "$@"'
logfile = '/tmp/threecolrefAppimageLog.txt'
...
content.extend([
    ...
    # See also https://github.com/rbreu/threecolref/issues/102
...
filename = 'org.threecolref.threecolref.appdata.xml'
...
Terminal=false
Exec=threecolref-{BEEVERSION}
Type=Application
Icon=logo

MimeType=application/x-threecolref;
Categories=Qt;KDE;Graphics;
X-KDE-NativeMimeType=application/x-threecolref
X-KDE-ExtraNativeMimeTypes=

X-AppImage-Version={BEEVERSION}
"""

filename = 'squashfs-root/usr/share/applications/org.threecolref.threecolref.desktop'
with open(filename, 'w') as f:
    f.write(content)
os.symlink('usr/share/applications/org.threecolref.threecolref.desktop',
           'squashfs-root/threecolref.desktop')

logger.info('Copying logos...')
shutil.copyfile('./threecolref/assets/logo.svg', 'squashfs-root/logo.svg')
shutil.copyfile('./threecolref/assets/logo.png', 'squashfs-root/.DirIcon')
...
run_command('./appimagetool.appimage',
            'squashfs-root',
            f'threecolref-{BEEVERSION}.appimage')
