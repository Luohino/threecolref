...
class BeeSettings(QtCore.QSettings):
    ...
    FIELDS = {
        ...
        'Save/remember_unsaved_choice': {
            'default': False,
            'cast': bool,
        },
        'Save/last_unsaved_choice': {
            'default': 'ask',
            'validate': lambda x: x in ('save', 'discard', 'ask'),
        },
    ...
    def valueOrDefault(self, key):
        ...
        opposed to settings that threecolref stores on its own.)
    ...
    def get_recent_files(self, existing_only=False):
        ...
        self.endArray()
        if existing_only:
            values = [v for v in values if os.path.exists(v)]
        return values
    ...
    def remove_recent_file(self, filename):
        ...
        if filename in values:
            ...
            # Remove the whole group to ensure the array is clean if it shrunk
