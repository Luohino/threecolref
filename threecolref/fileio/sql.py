...
"""threecolref's native file format is using SQLite. Embedded files are
...
from threecolref.items import BeePixmapItem, BeeErrorItem
...
def is_bee_file(path):
    """Check whether the file at the given path is a valid scene file."""
    return os.path.splitext(path)[1] in ('.bee', constants.EXTENSION)
...
class SQLiteIO:
    ...
    def _migrate(self):
        ...
        if self.readonly:
            try:
                ...
                tmpname = os.path.join(self._tmpdir.name, f'mig{constants.EXTENSION}')
    ...
    def read(self):
        ...
        for i, row in enumerate(rows):
            ...
            if data['type'] == 'pixmap':
                ...
                data['item'] = item
            elif data['type'] == 'video' and row[9]:
                # Extract video from storage to temp file
                from threecolref.items import BeeVideoItem
                import tempfile
                video_bytes = row[9]
                # Get the original filename from data
                orig_filename = data['data'].get('filename', 'video.mp4')
                _, ext = os.path.splitext(orig_filename)
                # Write to temp file
                temp_fd, temp_path = tempfile.mkstemp(suffix=ext)
                try:
                    os.write(temp_fd, video_bytes)
                    os.close(temp_fd)
                    item = BeeVideoItem(temp_path)
                    data['item'] = item
                except Exception as e:
                    os.close(temp_fd)
    ...
    @handle_sqlite_errors
    def write(self, thumbnail=None):
        if self.readonly:
            raise sqlite3.OperationalError(
                'Attempt to write to a readonly database')
        try:
            self.create_schema_on_new()
            self.write_data(thumbnail=thumbnail)
        except Exception:
            if self.retry:
                ...
                self.create_new = True
                self._close_connection()
                self.write(thumbnail=thumbnail)

    def write_data(self, thumbnail=None):
        ...
        if thumbnail:
            logger.debug("Saving thumbnail to metadata")
            self.ex('INSERT OR REPLACE INTO metadata (key, value) VALUES (?, ?)',
                    ('thumbnail', thumbnail))

        self.connection.commit()
        logger.debug(f'Calling VACUUM on {self.filename}')
        self.ex('VACUUM')
        # VACUUM implicitly commits, but ensure connection stays open and synced
        self.connection.commit()
        logger.debug(f'VACUUM and commit completed for {self.filename}')
        if self.worker:
            self.worker.finished.emit(self.filename, [])
    ...
    def insert_item(self, item):
        ...
        elif hasattr(item, 'video_to_bytes'):
            video_bytes, ext = item.video_to_bytes()
            if video_bytes:
                name = item.get_filename_for_export(ext)
                self.ex(
                    'INSERT INTO sqlar (item_id, name, mode, sz, data) '
                    'VALUES (?, ?, ?, ?, ?)',
                    (item.save_id, name, 0o644, len(video_bytes), video_bytes))
        self.connection.commit()
    ...
    def get_thumbnail(filename):
        """Quickly retrieve project thumbnail from a .3col file.
        
        Prioritizes the 'thumbnail' metadata blob.
        Falls back to the first available image in the sqlar table.
        """
        ...
        try:
            ...
            # Note: ?mode=ro ensures we don't accidentally write or lock unnecessarily
            with sqlite3.connect(f'{uri}?mode=ro', uri=True) as conn:
                cursor = conn.cursor()
                
                # 1. Try dedicated thumbnail first
                cursor.execute("SELECT value FROM metadata WHERE key='thumbnail'")
                row = cursor.fetchone()
                if row:
                    return row[0]
                
                # 2. Fallback: try first image in sqlar
                cursor.execute("SELECT data FROM sqlar LIMIT 1")
                row = cursor.fetchone()
                if row:
                    return row[0]
                    
        except Exception:
            # Table might not exist or file isn't openable
            pass
        return None
