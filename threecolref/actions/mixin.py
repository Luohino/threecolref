...
from .actions import Action, bee_actions
...
class ActionsMixin:
    ...
    def build_menu_and_actions(self):
        ...
        # Premium iOS-style Menu Aesthetics
        self.context_menu.setAttribute(Qt.WidgetAttribute.WA_TranslucentBackground)
        self.context_menu.setWindowFlags(self.context_menu.windowFlags() | Qt.WindowType.FramelessWindowHint | Qt.WindowType.NoDropShadowWindowHint)
        
        # High-end iOS Style Sheet
        ios_qss = """
            QMenu {
        ...
        """
        self.context_menu.setStyleSheet(ios_qss)
        ...
        self.context_menu.addAction(bee_actions['always_on_top'].qaction)
        self.context_menu.addAction(bee_actions['hierarchy'].qaction)
    ...
    def _create_actions(self):
        for action in bee_actions.values():
    ...
    def _create_menu(self, menu, items):
        ...
        for item in items:
            if isinstance(item, str):
                menu.addAction(bee_actions[item].qaction)
            if item == MENU_SEPARATOR:
                menu.addSeparator()
            if isinstance(item, dict):
                submenu = menu.addMenu(item['menu'])
                
                # Apply premium styling to submenus as well
                ...
                submenu.setStyleSheet(self.context_menu.styleSheet())
    ...
    def _clear_recent_files(self):
        ...
        for key in list(bee_actions.keys()):
            if key.startswith('recent_files_'):
                bee_actions[key].qaction = None
