...
from threecolref.config.controls import MouseConfig
from threecolref.widgets.controls.mouse import (
    MouseDelegate,
    MouseControlsEditor,
    MouseModel,
    MouseProxy,
...
def test_mouse_editor_inits_buttons_and_modifiers_when_not_configured(view):
    action = MouseConfig(
        ...
        modifiers=[],
        invertible=True)
    with patch('threecolref.config.controls.KeyboardSettings.MOUSE_ACTIONS',
               ActionList([action])):
...
@patch('threecolref.widgets.controls.mouse.MouseControlsEditor.accept')
