...
from threecolref.__main__ import threecolrefMainWindow, main
from threecolref.assets import BeeAssets
from threecolref.view import BeeGraphicsView


@patch('PyQt6.QtWidgets.QWidget.show')
def test_threecolref_mainwindow_init(show_mock, qapp):
    window = threecolrefMainWindow(qapp)
    assert window.windowTitle() == 'threecolref'
...
def test_threecolrefapplication_fileopenevent(open_mock, qapp, main_window):
    event = MagicMock()
...
@patch('threecolref.__main__.threecolrefApplication')
@patch('threecolref.__main__.CommandlineArgs')
@patch('threecolref.config.BeeSettings.on_startup')
