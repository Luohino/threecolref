...
def test_on_view_scale_change(view, item):
    with patch('threecolref.items.BeePixmapItem.prepareGeometryChange') as m:
...
@patch('threecolref.items.BeePixmapItem.draw_debug_shape')
...
def test_paint_when_debug_shapes(view):
    with patch('threecolref.selection.commandline_args') as args_mock:
        with patch('threecolref.items.BeePixmapItem.draw_debug_shape') as m:
