...
from threecolref.fileio import schema, is_bee_file
from threecolref.fileio.errors import BeeFileIOError
from threecolref.fileio.sql import SQLiteIO
from threecolref.items import BeePixmapItem, BeeTextItem, BeeErrorItem
...
def test_sqliteio_migrate_does_nothing_when_version_ok(tmpfile):
    ...
    del io
    with patch('threecolref.fileio.sql.SQLiteIO.ex') as ex_mock:
        SQLiteIO(tmpfile, MagicMock())
        ex_mock.assert_not_called()


@patch('threecolref.fileio.sql.USER_VERSION', 3)
@patch('threecolref.fileio.sql.MIGRATIONS', {
...
@patch('threecolref.fileio.sql.SQLiteIO._migrate')
